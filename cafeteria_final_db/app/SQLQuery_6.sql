...
CREATE TABLE Productos (
    ...
    -- AJUSTE: Añadimos la columna 'stock' para que funcione con la aplicación web CRUD.
    -- Esto representa el stock de productos finales listos para la venta.
...
-- NOTA: Se ha actualizado la tabla Productos con un stock inicial
INSERT INTO Productos (nombre, descripcion, precio, tipo_producto, stock) VALUES
('Espresso', 'Shot de café concentrado.', 2.50, 'Bebida', 150), ('Latte Vainilla', 'Espresso con leche y sirope de vainilla.', 4.00, 'Bebida', 120),
('Muffin Choco', 'Muffin de chocolate casero.', 3.50, 'Comida', 60), ('Croissant', 'Croissant de mantequilla.', 2.00, 'Comida', 80),
('Capuchino', 'Café con leche espumada.', 3.80, 'Bebida', 100), ('Jugo Naranja', 'Jugo de naranja natural.', 3.00, 'Bebida', 50),
('Brownie', 'Brownie de nueces.', 4.50, 'Comida', 45), ('Té Verde', 'Té verde premium.', 2.80, 'Bebida', 90),
('Sándwich Pavo', 'Sándwich de pavo y queso.', 6.00, 'Comida', 30), ('Donut Glaseado', 'Donut con glaseado de azúcar.', 2.50, 'Comida', 75);
...
GO
-- INSERT (C) - SP_InsertarProducto ajustado para incluir 'stock'
...
AS
BEGIN
    INSERT INTO Productos (nombre, descripcion, precio, tipo_producto, stock)
    VALUES (@p_nombre, @p_descripcion, @p_precio, @p_tipo_producto, @p_stock);
END;
GO
...
-- UPDATE (U) - SP_ActualizarPrecioProducto ajustado para actualizar stock también
...
AS
BEGIN
    -- Primero elimina registros dependientes si existieran (ejemplo: recetas)
    DELETE FROM Detalle_Receta WHERE receta_id IN (SELECT receta_id FROM Recetas WHERE producto_id = @p_id);
    DELETE FROM Recetas WHERE producto_id = @p_id;
...
SELECT nombre_ingrediente, stock_actual, stock_minimo, unidad_medida FROM Ingredientes WHERE stock_actual <= stock_minimo;
GO
